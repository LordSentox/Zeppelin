...
void adc_init_inputs() {
	...
	// as to make up a Voltage Divider between those Values.
...
int adc_do_conversion() {
	...
	ADC10AE0 &= ~(BIT2 + BIT3 + BIT3);			// Clean up the current inputs.
	ADC10CTL0 &= ~ENC;							// End the current Conversion.
	return ADC10MEM;
}
